...
export function resolveModelWithTier(requestedModel: string, options: ModelResolverOptions = {}): ResolvedModel {
  ...
  if (skipAlias) {
    ...
    } else if (isGemini3Flash && GEMINI_3_8_FLASH_REGEX.test(modelWithoutQuota)) {
      antigravityModel = resolveGemini38FlashModelForLevel(baseName, tier);
...
export function resolveGemini36FlashModelForLevel(
  actualModel: string,
  level: string | undefined,
): string {
  const isGemini36Flash = GEMINI_3_6_FLASH_REGEX.test(actualModel);
  if (!isGemini36Flash) {
    return actualModel;
  }
  if (level === "high") return "gemini-3.6-flash-high";
  if (level === "medium") return "gemini-3.6-flash-medium";
...
export function resolveGemini38FlashModelForLevel(
  actualModel: string,
  level: string | undefined,
): string {
  const isGemini38Flash = GEMINI_3_8_FLASH_REGEX.test(actualModel);
  if (!isGemini38Flash) {
    return actualModel;
  }
  if (level === "high") return "gemini-3.8-flash-high";
  if (level === "medium") return "gemini-3.8-flash-medium";
...
export function isGemini38FlashModel(model: string): boolean {
...
export function resolveModelWithVariant(
  ...
  if (isGemini3) {
    ...
    if (isAntigravityGemini3Pro) {
      ...
      actualModel = resolveGemini38FlashModelForLevel(actualModel, level);
    }
